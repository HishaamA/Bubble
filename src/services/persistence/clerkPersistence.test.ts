...
describe('Clerk-backed persistence service', () => {
  ...
  it('reads an approved family membership scoped to the internal identity', async () => {
    mocks.membership.maybeSingle.mockResolvedValue({
      data: { circle_id: circleId, role: 'owner' },
      error: null,
    })
    mocks.circles.single.mockResolvedValue({
      data: { name: 'The Ahmed family' },
      error: null,
    })
    ...
    expect(mocks.membership.eq).toHaveBeenCalledWith(
      'user_id',
      internalUserId,
    )
  })

  it('creates a family and joins another only after profile bootstrap', async () => {
    await expect(createFamily('  The Ahmed family  ')).resolves.toEqual({
      id: circleId,
      name: 'The Ahmed family',
    })
    expect(mocks.circles.insert).toHaveBeenCalledWith({
      name: 'The Ahmed family',
